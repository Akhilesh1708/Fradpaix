...
/* ---- Fetch leads from Google Sheets via JSONP (bypasses CORS) ---- */
function syncLeadsFromSheets(callback) {
  ...
  // Use JSONP — Apps Script supports ?callback= natively, no CORS issues
  const cbName = '_fradpaixCb_' + Date.now();
  ...
  const finish = (success) => {
    ...
    if (indicator) {
      indicator.textContent = success ? '✓ Synced' : '✗ Sync failed';
      indicator.style.color = success ? '' : '#e69090';
      setTimeout(() => { indicator.style.display = 'none'; indicator.style.color = ''; }, 2000);
    }
    callback && callback();
  };

  const timer = setTimeout(() => {
    console.warn('Sheets sync timed out');
    finish(false);
  }, 10000);

  window[cbName] = function(data) {
    try {
      if (!data.ok || !Array.isArray(data.leads)) throw new Error('Bad response');

      const remote   = data.leads.filter(l => l.name);
      const local    = getCrmLeads();
      const localMap = {};
      local.forEach(l => { if (l.id) localMap[l.id] = l; });

      remote.forEach(r => {
        if (localMap[r.id]) {
          localMap[r.id] = { ...r, status: localMap[r.id].status };
        } else {
          localMap[r.id] = r;
        }
      });
      ...
      console.error('Sheets sync parse error:', e);
      finish(false);
    }
  };

  script.onerror = () => { console.error('Sheets sync script load failed'); finish(false); };
  script.src = url + '?type=leads&callback=' + cbName;
